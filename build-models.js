...
// Read the ES module build of model-viewer (no network dependency)
...
async function main() {
  ...
  for (const name of files) {
    ...
    const modelRel = `assets/models/${name}`;
    ...
    // Load metadata and override with detected files
    const meta = JSON.parse(fs.readFileSync(metaFile, 'utf8'));
    meta.source = modelRel;
    meta.poster = posterRel;

    let changed = false;
    // Auto-detect additional formats if not already set
    if (!meta.usdz) {
      const usdzPath = path.join(MODELS_DIR, `${id}.usdz`);
      if (fs.existsSync(usdzPath)) {
        meta.usdz = `assets/models/${id}.usdz`;
        changed = true;
      }
    }
    if (!meta.fbx) {
      const fbxPath = path.join(MODELS_DIR, `${id}.fbx`);
      if (fs.existsSync(fbxPath)) {
        meta.fbx = `assets/models/${id}.fbx`;
        changed = true;
      }
    }

    // Save metadata if we injected new fields
    if (changed) {
      fs.writeFileSync(metaFile, JSON.stringify(meta, null, 2));
      console.log(`Updated metadata with detected formats: ${metaFile}`);
    }
